...
// Read the shift register and update the selected CTCSS code,  PTT and tone status
void readShiftRegister() {
  ...
  for (int i =0; i < numCodes; i++) {
    if ((shiftRegister & 0b111111) == ctcssCodes[i]) {
      ...
      Serial.print("CTCSS: ");
      Serial.print(ctcssFrequencies[i]);
      Serial.println(" Hz");
      }
      break;
    }
